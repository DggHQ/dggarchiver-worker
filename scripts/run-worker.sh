...
case "$LIVESTREAM_PLATFORM" in
	...
	"kick" )
		...
		if [ "${LIVESTREAM_DOWNLOADER}" = "yt-dlp" ]; then
			...
			if [ "${DOWNLOAD_PROXY}" = "" ]; then
				streamlink --stream-segment-attempts 25 --stream-timeout 600 --hls-playlist-reload-attempts 50 -o "/videos/${LIVESTREAM_PLATFORM}_${LIVESTREAM_ID}.mp4" "${LIVESTREAM_URL}" "${QUALITY}"
			else
